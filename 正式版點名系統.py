...
import 距離1196
import 座標1196
import 角度1307  # 更改為1307
import 距離1307  # 更改為1307
import 座標1307  # 更改為1307
import 畫圖2
...
def measure_and_display_result(n, root, image_label, result_label, coord_label):
    ...
    while i < n:
        ...
        # 檢測 tag 1307  # 更改為1307
        a1307 = math.radians(角度1307.angle1307())  # 更改為1307
        d1307 = 距離1307.distance1307()  # 更改為1307
        x307, y307, x1307, y1307, x13072, y13072 = 座標1307.xy1307(d1307, a1307)  # 更改為1307
        print(f"1307座標: ({x307}, {y307})")  # 更改為1307
        
        # 繪製 tag 1307 的座標 繪製 tag 1196 的座標
        畫圖2.a1(x307, y307, x196, y196)  # 更改為1307
        
        coord_label.config(text=f"1307座標: ({x307}, {y307})\n1196座標: ({x196}, {y196})")

        if 2 < x307 < 3 and 1 < y307 < 7:   # 設定 1307 座位範圍  # 更改為1307
            flag1307 = 1  # 更改為1307

        if flag1196 == 1 and flag1307 == 1:  # 更改為1307
            break
    ...
    result_text += "\n"
    result_text += "1307到課" if flag1307 == 1 else "1307未到課"  # 更改為1307
